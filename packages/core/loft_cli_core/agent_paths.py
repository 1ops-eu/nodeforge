"""Server-side path constants for the loft-cli agent.
...
AGENT_CONFIG_DIR = Path("/etc/loft-cli")

# Runtime state (mutable: locks, state tracking, desired state)
AGENT_STATE_DIR = Path("/var/lib/loft-cli")
...
# Logs
AGENT_LOG_DIR = Path("/var/log/loft-cli")

# Agent binary location
AGENT_BINARY_PATH = Path("/usr/local/bin/loft-cli-agent")
