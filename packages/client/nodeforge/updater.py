...
def self_update(console: Console | None = None) -> bool:
    ...
    if not url:
        c.print(
            ...
            f"Try: pip install --upgrade nodeforge[/yellow]"
        )
        return False
    ...
    try:
        ...
        if current_binary.name == "python" or current_binary.name.startswith("python"):
            c.print(
                ...
                "use pip install --upgrade nodeforge instead[/yellow]"
            )
            return False
...
def update_agent(transport: Transport, console: Console | None = None) -> bool:
    ...
    from nodeforge_core.agent_paths import AGENT_BINARY_PATH
    ...
    try:
        ...
        transport.upload(tmp_local, "/tmp/nodeforge-agent.tmp")
        transport.run(f"mv /tmp/nodeforge-agent.tmp {AGENT_BINARY_PATH}", sudo=True, warn=True)
